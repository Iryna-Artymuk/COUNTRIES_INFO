...
ref.input.addEventListener('input',debounce(getCountryOnInput,DEBOUNCE_DELAY ))
...
fetchCountries(countryName).then(data=>{
 ...
 function generateMarkupList(data){
    console.log(data)
    if(data.length===1){
        const markup = data.map(({flags,name:countryName,capital,population,languages})=>
           ...
           <div><img src=" ${flags.svg}" alt="" width="200"></div>
                ...
                <ul>
                  ...
                  <li><p class="countryInfo"> Population: ${numberWithSpaces(population)}</p></li> 
                   <li><p class="countryInfo">Language:  ${languages.map(item=>item.name)}</p></li> 
                </ul>
...
if(data.length>1&&data.length<10){
    ...
    const markup = data.map(({flags,name:countryName,})=>
 ...
 function getFullInfoOnClick(event){
